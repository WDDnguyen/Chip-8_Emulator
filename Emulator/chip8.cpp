...
double chip8::clockTimer()
{
	end_timer = clock();
	//time(&end_timer);
	//double elapsed_sec = difftime( double(end_timer),double (start_timer));
	double elapsed_sec = ((double)(end_timer - start_timer) / CLOCKS_PER_SEC);
	//printf("%d\n", elapsed_sec);
	return elapsed_sec;
}
...
{
	opcount += 1;
	double numberOfOperations = double(opcount) / ops;
	//printf("number of opcode per seconds : %f /n", numberOfOperations);
	//Sleep(1.5);
	//printf("Opcount : %d   Ops : %d   numberOfOperations : %f  elapsedTime: %f \n ", opcount, ops, numberOfOperations, clockTimer());
	 
	while (numberOfOperations > clockTimer()){
		printf("Opcount : %d   Ops : %d   numberOfOperations : %f  elapsedTime: %f \n ", opcount,ops,numberOfOperations,clockTimer());
		Sleep(15);
	}  
